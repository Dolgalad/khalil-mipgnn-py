...
def mipeval(
    ...
    summary_string.write('solving stats,%s,%g,%g,%g,%g,%i,%g,%s,%i,%i,%g,%g\n' % (
            ...
            time_vcg_reading,
            time_pred))
    print(f"cplex_status              : {cplex_status}")
    print(f"best_objval               : {best_objval}")
    print(f"best_bound                : {best_bound}")
    print(f"gap                       : {gap}")
    print(f"total_time                : {total_time}")
    print(f"num_nodes                 : {num_nodes}")
    print(f"timelimit - time_rem_cplex: {timelimit - time_rem_cplex}")
    print(f"instance_name             : {instance_name}")
    print(f"num_variables             : {num_variables}")
    print(f"num_constraints           : {num_constraints}")
    print(f"time_vcg_reading          : {time_vcg_reading}")
    print(f"time_pred                 : {time_pred}")

    # solve with cplex for comparison
    from mipgnn import bias_search
    default_ip = cplex.Cplex()
    bias_search.disable_output_cpx(default_ip)
    default_ip.parameters.timelimit.set(timelimit)
    default_ip.parameters.emphasis.mip.set(cpx_emphasis)
    default_ip.parameters.mip.display.set(3)
    default_ip.parameters.threads.set(cpx_threads)
    default_ip.parameters.workmem.set(memlimit)
    default_ip.parameters.mip.limits.treememory.set(20000)
    default_ip.parameters.mip.strategy.file.set(2)
    default_ip.parameters.workdir.set(cpx_tmp)
 
    default_ip.read(instance)
    t0 = default_ip.get_time()
    default_ip.solve()
    t0 = default_ip.get_time() - t0
    print(f"cplex_time                : {t0}")
    print(f"speedup                   : {t0 / total_time}")
    ...
    return t0 / total_time, gap, default_ip.solution.MIP.get_mip_relative_gap()

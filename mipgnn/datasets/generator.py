...
def generate_instances(name, n_constraint=500, n_variable=500, density=0.2, n_instance=1000, dataset_directory=config.dataset_directory, generator=ecole.instance.SetCoverGenerator, train_test_split=0.0, process=True):
    ...
    # dump the dataset description into the folder
    dataset_info = {
            "name": name,
    ...
    dataset_info_path = os.path.join(path, "{name}.json")
    with open(dataset_info_path, "w") as f:
        f.write(json.dumps(dataset_info, indent=4, sort_keys=True))
